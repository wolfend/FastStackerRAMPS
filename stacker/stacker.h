...
// Requires RAMPS 1.4 HW. This is an adaptation of 1.16 to run on Arduino Mega/RAMPS/Full Graphic controller patform
//The main change was replacing the display and associated drivers. Now uses portable u8g2lib graphics library installed using Arduino
//library manager.
...
// DW 6/17/17- set to "long" to accomodate 220mm / 8mm/rev X 200 steps/rev X 8 microsteps/step = 44,000 microsteps (BTW, 8mm/rev / 200 steps/rev * 8 microsteps/step = .005mm/microstep) 
...
//RAMPS LCD stuff
const short BEEPER = 37;
const short PIN_CLOCK = 23;
const short PIN_DATA = 17;
const short PIN_CS = 16;
// Create an u8g2 lcd object (do it here by the pin defs so these lines all can be copied to use in other programs)
U8G2_ST7920_128X64_F_SW_SPI lcd(U8G2_R0, PIN_CLOCK, PIN_DATA, PIN_CS, /* reset=*/ U8X8_PIN_NONE);

//We will be using the u8g2_font_t0_11_mf font from ug2lib which leads to a text display of 6 rows and 21 columns. Rows/columns numbered starting at 0. To convert
//row or column numbers to pixel position for plotting using drwStr, multiply by the constants below
const short ROW_Y = 10;
const short COL_X = 6;

//stepper driver using X driver on RAMPS board
const short PIN_STEP = 54;
const short PIN_DIR = 55;
const short PIN_ENABLE = 38;  // LOW: enable motor; HIGH: disable motor (to save energy)

//limit switches using X limit inputs. These are logically OR'd
const short PIN_LIMITER_MIN = 3;
const short PIN_LIMITER_MAX = 2;


//Camera control pins use pins on the AUX2 connector (using D0/D1 on AUX1 interferred with serial uploading)
// Pin to trigger camera shutter
const short PIN_SHUTTER = 40;
// Hardware pin to operate the AF relay:
const short PIN_AF = 63;

//////// Voltage parameters: ////////
// Analog pin for the battery voltage. This is the Y-DIR pin on the Y stepper driver header on RAMPS (pin 8 of header)
//Form voltage devider between pin 9 (gnd) and pin 16 (Vin) of this same header.
#define PIN_BATTERY A7
const short NUM_CELLS = 6; //number of AA NiMH cells
// Scaling coefficient to derive the battery voltage (depends on the resistance of the two dividing resistors, R3 and R4.
// Assuming R3 is the one directly connected to "+" of the battery (header pin16), the scaler is (R3+R4)/R4. R3+R4 should be ~0.5M)
//For RAMPS, assume a nominal input voltage range of 7.4V to 9V, we will scale 9.2V to 2.56V with these resistors:
//for RAMPS R4=150K, R3= 390K, (15+39)/15=3.6 voltage scale factor (calculated)
//After building divider, measure the input V and the divider tap and divide the two to determine acutal voltage scale
...
const float V_HIGH = 1.4; //determined by experiment- Higher than this causes low battery error even w/ok battery
// Critically low voltage, per AA battery (when V becomes lower than this, the macro rail is disabled)
const float V_CRITICAL = 1.1;  //determined by experiment- Higher than this causes low battery error even w/ok battery
...
const float SPEED_VOLTAGE = 1.25; //Set to the cell voltage you want as lower limit for full speed operation
//////// Keypad stuff: ////////
...
//RAMPS: keypad keys connected to servo connector, to ymin, ymax, zmin, zmax endstop pins
//Pins 1-4 of keypad go to servo pins, pins 5-8 go to endstop pins
//Keyboard mappings for common red/blue membrane keyboard.
//byte rowPins[rows] = {11, 6, 5, 4}; //connect to the row pinouts of the keypad, these are the servo pins
//byte colPins[cols] = {14, 15, 18, 19}; //connect to the column pinouts of the keypad
//Keyboard mappings for common black/white pushbutton keypad
byte rowPins[cols] = {14, 15, 18, 19}; //row pinouts of the keypad, these are the Ymin, Ymax, Zmin, Zmax endstop pins
byte colPins[rows] = {11, 6, 5, 4}; //column pinouts of the keypad, these are the servo pins on the RAMPS
...
// Number of microsteps in a step (all jumpers in place for stepper driver on RAMPS):
const COORD_TYPE N_MICROSTEPS = 16;

// Macro rail parameter: travel distance per one rotation, in mm (8.00mm for 8mm lead screw Slider): 
const float MM_PER_ROTATION = 8.00;

//Maximum length of macro rail
const float RAIL_LENGTH = 180.0; //used to check reasonable #'s of frames for 1pt stack
...
struct global
{
  ...
  char buffer[22];  // char buffer to be used for lcd print; 1 more element than the lcd width (21)
  ...
  char buf6[6]; // Buffer to store the stacking length for displaying (also used in status to hold timelapse stack count before displaying DW)
  char buf7[8]; // (buffer used to store current position for displaying DW- increased from 7 to 8)
